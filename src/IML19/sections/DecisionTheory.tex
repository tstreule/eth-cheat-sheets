...
giv.: $P(y\vert x)$, set of actions $\mathcal{A}$, cost $C\!\!:\mathcal{Y}\!\times\!\mathcal{A} \!\to\! \mathbb{R}$

\textbf{opt. action:} $a^\ast = \arg\!\min\limits_{a\in\mathcal{A}} \mathbb{E}_y [C(y,a)\vert x]$\\
\phantom{\emph{opt. action}: $a^\ast$}
$= \arg\!\min\limits_{a} \sum_n P(y\vert x)\, C(y,a)$
...
\phantom{E.g. }$c_+ = \mathbb{E}_y (C(y,+1)\vert x) = (1-p) c_{FP}$\\
\phantom{E.g. $c_+$}$\;= P(y \!=\! 1\vert x) \cdot 0 + P(y \!=\! -1\vert x) \cdot c_{FP}$
 
\textbf{Regression:}  $C(y,a) = (y-a)^2$; asymmetric: \\
